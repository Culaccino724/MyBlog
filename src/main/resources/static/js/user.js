...
$('.userList .clickLi').click(function () {
    ...
    $('#personalDate,#basicSetting,#commentMessage,#leaveMessage,#leaveWord,#privateWord,#articleManagement,#articleThumbsUp').css("display","none");
        ...
        $.each(date['result'], function (index, obj) {
            ...
            if(obj['pId'] == 0){
                msgRead.append($('<span class="msgHead">' +
                    '<a class="msgPerson">' + obj['answerer'] + '</a>评论了你的博客' +
                    '</span>'));
            } else {
                msgRead.append($('<span class="msgHead">' +
...
function getArticleManagement(currentPage) {
    $.ajax({
        type:'post',
        url:'/getArticleManagementByUser',
        dataType:'json',
        data:{
            rows:10,
            pageNum:currentPage
...
function putInArticleThumbsUp(data) {
    ...
    if(data['result'].length == 0){
        ...
        $.each(data['result'], function (index, obj) {
            ...
            msgRead.append($('<span class="am-badge msgType">收藏</span>'));
            msgRead.append($('<span class="msgHead"><a class="msgPerson">' + obj['praisePeople'] + '</a>收藏了你的博客</span>'));
            msgRead.append($('<div class="msgTxt">' +
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data['status'] == 101){
                ...
                dangerNotice(data['message'] + " 获得收藏失败")
            } else {
